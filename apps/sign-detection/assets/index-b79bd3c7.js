...
class SignDetector {
  constructor(threadCount = 1, signDetectorWorker = SignDetectorWorker) {
...
function useSignDetector() {
  ...
  reactExports.useEffect(() => {
    async function createSignDetector() {
      const threadCount = 3;
      const signDetector2 = new SignDetector(threadCount, WorkerWrapper);
...
const WebcamWrapper = reactExports.forwardRef(
  ({ onVideoDimensions }, ref) => {
    ...
    const webcamVideo = (_a = webcamRef.current) == null ? void 0 : _a.video;
    const [videoWidth, setVideoWidth] = reactExports.useState(0);
    const [videoHeight, setVideoHeight] = reactExports.useState(0);
    if (webcamVideo) {
      webcamVideo.onplay = () => {
        const { videoWidth: videoWidth2, videoHeight: videoHeight2 } = webcamVideo;
    ...
    const canvasContext = reactExports.useMemo(() => {
      var _a2;
      const video = (_a2 = webcamRef.current) == null ? void 0 : _a2.video;
      if (!video)
        return;
      if (videoWidth <= 0)
        return;
